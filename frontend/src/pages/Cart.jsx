  ...
  const getTotal = (items, cartItems, rest) => {
    ...
    cartItems.map((cartItem) => {
      const matchingItem = items.find(
        (item) => item.id === cartItem.item_id && item.restaurant_id === rest
      );
      if (matchingItem) {
    ...
    api
      ...
      .then((data) => {
        ...
        getTotal(
          ...
          data.cart_items,
          rest !== 0
            ? data.rests.find((r) => r.id === rest).id
            : data.rests[0].id
        );
      });
  };
    ...
    return (
      <div id="cart">
        ...
        <div id="main">
          ...
          <div
            style={creatingOrder ? { display: "none" } : { display: "block" }}
          >
            <MakeOrder
              rests={rests}
              test={test}
              createOrder={setCreatingOrder}
              items={items}
              subtotal={total ? total : 0}
              changeItems={setOneRestItems}
              updateTotal={(items, restId) =>
                getTotal(items, cartItems, restId)
              }
              updateRest={setRest}
            />
          </div>
      ...
      <div id="cart">
        ...
        <div id="main">
          <div id="empty">
            ...
            <h2>
              You can do it in <a href="/menu">menu</a>
            </h2>
          </div>
        </div>
      </div>
